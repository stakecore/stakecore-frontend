...
import { AVALANCHE_COLOR_CODE, avalanchePChainAddressUrl } from "~/constants"
...
const AvalancheValidatorLocalDelegateComponent = () => {
  ...
  const pchain = pubk != null ? publicKeyToPAddress('avalanche', pubk) : undefined

  const { data: resp, isLoading, error } = useSWR(
    ['avalanche-p-chain-address', walletAddress, pchain], ([_, cchain, pchain]) => {
      if (cchain == null || resp?.status == 404 && pchain == null) {
        throw new Error('not ready')
      }
      return AvalancheValidatorDataAccess.getDelegatorInfo(cchain, pchain)
    }, {
  ...
  if (walletAddress == null) {
    ...
    component = <div style={{ textAlign: 'center' }}>
      <SpinnerCircular color={AVALANCHE_COLOR_CODE} size={45} />
    </div>
    ...
    component = <div>
      <div>
        <p>
          ...
          This means you never interacted with the P-chain on Avalanche.
    ...
    component = <>
      <div>
        ...
        <div className="avalanche-delegator-list-container mt-40">
          <p>
