...
[.githooks/pre-push](.githooks/pre-push) runs five checks against whatever `git push` is about to send: **shellcheck** over every shell script, **gitleaks** over every commit the remote does not have yet, **actionlint** over the workflows (which also runs shellcheck on each `run:` block), **zizmor** for workflow security, and **`pnpm audit`**, which fails on `high` or worse. `post-create.sh` sets `core.hooksPath` to `.githooks`. That setting lives in `.git/config`, so git on the host runs the hook too, and `audit` there needs a `pnpm`. Any other Linux checkout needs `git config core.hooksPath .githooks` once. `SKIP=audit,zizmor git push` skips named checks and says so in the output; `--no-verify` skips the hook. These checks run only on your machine — CI does not run them, and the hook is one flag from bypassed.

- **The checks read the pushed commits, not the working tree.** Each pushed tip is written out to a temp dir through a throwaway index (`read-tree` + `checkout-index`). So a dirty file cannot wave through a broken commit, and `git push origin other-branch` checks that branch, not the one checked out. gitleaks gets the pushed commits minus everything reachable from `refs/remotes/<remote>/*` and the remote's current tips. Pushing commits that are already upstream therefore checks nothing, and a push to a bare URL (no tracking refs) scans all of history, which takes under a second on this repo.
- **gitleaks exits 0 when git fails underneath it, so its exit code alone proves nothing.** It splits `--log-opts` on spaces and hands the pieces to `git log`. The first version of the hook left a trailing space there whenever the push had no remote-tracking refs to exclude, so git got an empty argument and died with `ambiguous argument ''`. gitleaks still printed `0 commits scanned` and `no leaks found`, exited 0, and the hook reported *all checks passed* for a push whose commits were never scanned. An invalid revision does the same. `check_gitleaks` now builds the options with no empty pieces, proves the revisions to `git rev-list` first, and fails on any `ERR` log line or a missing `commits scanned` summary. Keep all three if you touch it.
- **Tools are pinned and hash-verified in [tools.sh](.githooks/tools.sh)**, and installed on first use into `~/.cache/stakecore-hooks/<tool>-<version>/`. The version is in the path, so a bump takes effect on the next push without anyone re-running anything. Take each new sha256 from GitHub's release-asset digest or the project's checksums file, never from your own download. A download that fails or does not match fails that check rather than skipping it. Only Linux x86_64 and aarch64 binaries are pinned.
- **zizmor runs `--offline`** so its verdict cannot depend on whether a `GH_TOKEN` happens to be set. The cost is the audits that need the GitHub API (`known-vulnerable-actions`, `impostor-commit`, …). Its policy is in [.github/zizmor.yml](.github/zizmor.yml): `actions/*` may float on a major tag, and every other action must be pinned to a commit. zizmor's own default pins GitHub's actions too, but the repo has no Dependabot config, so nothing would ever move those pins and `actions/checkout` would silently stop receiving its fixes. Adopt that default together with Dependabot, not on its own.
- **Two zizmor findings are ignored inline, not fixed:** the workflow-level `pages: write` and `id-token: write` in `deploy-site.yml`, whose comment there gives the reason. In short, the narrower grant belongs on the `deploy` job, `build` runs `actions/configure-pages`, and the workflow only runs on main, so the first proof would be a production deploy. `checkout`'s credential persistence (`artipacked`) *was* fixed in both workflows, since nothing after checkout talks to the remote.
- **The audit needs the network and judges the whole lockfile, not the diff.** A newly published advisory blocks a push that touched no dependency. That is intended; `SKIP=audit` is the way through while the bump lands.
